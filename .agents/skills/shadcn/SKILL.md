---
name: shadcn
description: Manages shadcn components and projects — adding, searching, fixing, debugging, styling, and composing UI. Provides project context, component docs, and usage examples. Applies when working with shadcn/ui, component registries, presets, --preset codes, or any project with a components.json file. Also triggers for "shadcn init", "create an app with --preset", or "switch to --preset". All command invocations must specify the --cwd flag to point to the `pacakges/design-system` directory
...
> **IMPORTANT:** Run all CLI commands using the project's package runner: `pnpm dlx shadcn@latest`, `pnpm dlx shadcn@latest`, or `bunx --bun shadcn@latest` — based on the project's `packageManager`. Examples below use `pnpm dlx shadcn@latest` but substitute the correct runner for the project.

## Current Project Context

```json
!`pnpm dlx shadcn@latest info --json --cwd`
```
...
- **Accessibility Links:** Always link `FieldDescription` or `FieldError` to the input control using `aria-describedby="<id>"`.
...
- **Toast via `sonner`.** Use `toast.success()` and `toast.error()` from `sonner` alongside `useMutation` for consistent user feedback on write operations.
