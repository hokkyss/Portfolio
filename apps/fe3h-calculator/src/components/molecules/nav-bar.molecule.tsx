...
const NAV_LINKS = [
  { label: 'Growths & Stats', to: '/growths-and-stats' as const },
];

/**
 * Fixed top navigation bar for the FE3H Calculator.
 */
      ...
      >
        ...
        <ul className={tw`hidden items-center gap-6 md:flex`} role="list">
          {NAV_LINKS.map((link) => (
            <li key={link.label}>
              <Link
                activeProps={{ className: tw`text-foreground font-semibold` }}
